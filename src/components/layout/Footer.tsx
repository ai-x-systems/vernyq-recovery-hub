      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12 py-16 lg:py-20">
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h3 className="text-overline text-[#faf9f7]/50 mb-4">{category}</h3>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-body-sm text-[#faf9f7]/70 hover:text-[#faf9f7] transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div className="max-w-md">
              ...
              <p className="text-body-sm text-[#faf9f7]/50 mt-3 leading-relaxed">
                Premium cold plunge systems engineered for home recovery. Designed to perform. Built to last.
              </p>
            </div>
        ...
        <div className="border-t border-[#faf9f7]/10 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-caption text-[#faf9f7]/40">© {new Date().getFullYear()} VERNYQ. All rights reserved.</p>
          <p className="text-caption text-[#faf9f7]/40">Premium recovery systems. Made for the dedicated.</p>
        </div>
      </div>
    </footer>
  );
