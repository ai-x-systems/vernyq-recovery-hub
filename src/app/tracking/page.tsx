...
export default function TrackingPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (orderNumber && email) setSubmitted(true);
  };
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-xl pb-20">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" className="flex items-center justify-center gap-2 h-12 px-8 bg-[#0A182E] text-[#faf9f7] text-body-sm font-medium rounded-[0.5rem] hover:bg-[#0A182E]/90 transition-colors">
          ...
          {submitted && (
            <div className="mt-10 bg-[#f3f1ee] border border-[#e0ddd8] rounded-[0.5rem] p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <p className="text-body font-medium text-[#0A182E]">Order {orderNumber}</p>
                  <p className="text-caption text-[#888888]">Last updated: checking status...</p>
                </div>
              </div>
              <div>
                {statuses.map((status, i) => (
                  <div key={i} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`size-3 rounded-full shrink-0 ${i === 0 ? "bg-[#4a8a5c]" : "bg-[#e0ddd8]"}`} />
                      {i < statuses.length - 1 && <div className="w-px h-8 bg-[#e0ddd8]" />}
                    </div>
                    <div className="pb-4">
                      <p className={`text-body-sm ${i === 0 ? "text-[#0A182E] font-medium" : "text-[#888888]"}`}>{status.label}</p>
                      {i === 0 && <p className="text-caption text-[#888888] mt-0.5">{status.description}</p>}
                    </div>
                  </div>
                ))}
              </div>
